...
async function syncChat(peer:number,by:number){
 ...
 const record={chatName:settings?.title??`Беседа ${peer}`,ownerId:Number(owner?.member_id??owner?.memberId??settings?.owner_id??0),syncedBy:by,syncedAt:Date.now()};
 await redis.set(key("sync",peer),record);await redis.sadd(key("synced_chats"),String(peer));return record;
}

async function handle(m:any){
 if(!m||typeof m!=="object"){console.log(`[MESSAGE] invalid type=${typeof m}`);return}
 const peer=Number(m.peer_id??0),user=Number(m.from_id??m.user_id??0);
 console.log(`[MESSAGE RAW] peer_id=${m.peer_id??"missing"} from_id=${m.from_id??"missing"} cmid=${m.conversation_message_id??"missing"} text=${JSON.stringify(m.text??"")}`);
 if(!isChatPeer(peer)||user<=0){console.log(`[MESSAGE IGNORE] peer=${peer} reason=not_chat_or_invalid_user`);return}
 const text=String(m.text??"").trim();if(!text.startsWith("/"))return;
 ...
 if(cmd==="/help"){const r=await resolveUserRole(peer,user,null),all=["/info","/help","/stats"];if(r.weight>=40)all.push("/setnick","/removenick","/getnick","/getacc","/nlist","/staff","/getban","/clear");if(r.weight>=50)all.push("/mute","/unmute");if(r.weight>=60)all.push("/addmoder","/delmoder","/timeout");if(r.weight>=70)all.push("/ban","/unban","/banlist");if(r.weight>=80)all.push("/sban","/sunban","/skick");if(r.weight>=90)all.push("/gban","/gunban","/gkick");return reply(peer,m,"Список доступных вам команд:\n\n"+all.join("\n"))}
...
Deno.serve(async(req)=>{try{
 ...
 const expected=Deno.env.get("VK_SECRET");
 if(expected&&body?.secret!==expected){console.error(`[WEBHOOK] secret mismatch: body_secret_present=${!!body?.secret}`);return new Response("bad",{status:403})}
 if(body?.type!=="message_new")return new Response("ok");
 let object=body?.object;
 console.log(`[WEBHOOK] body_object_type=${typeof object}`);
 if(typeof object==="string"){try{object=JSON.parse(object)}catch(e){console.error("[WEBHOOK] object JSON parse failed",e);return new Response("ok")}}
 console.log(`[WEBHOOK] object_keys=${object&&typeof object==="object"?Object.keys(object).join(","):"none"}`);
