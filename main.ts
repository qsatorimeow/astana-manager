...
async function ensureChat(peer:number){if(!isChat(peer))return;await S.setChatInfo(peer,await chatName(peer),await owner(peer));}
...
async function applyKickBan(peer:number,user:number,scope:string,by:number,reason:string){await S.addBan(scope,user,by,reason);if(isChat(peer)&&scope===String(peer))await kick(peer,user)}
...
if(c==="/mygroups"){
 ...
 if(c==="/addsa"||c==="/delsa"||c==="/addzsa"||c==="/delzsa"||c==="/addsenadmin"||c==="/delsenadmin"||c==="/addadmin"||c==="/deladmin"||c==="/addsenmoder"||c==="/delsenmoder"||c==="/addmoder"||c==="/delmoder"){
  const map:any={"/addsa":["developer","sa",1],"/delsa":["developer","sa",0],"/addzsa":["sa","zsa",1],"/delzsa":["sa","zsa",0],"/addsenadmin":["zsa","senadmin",1],"/delsenadmin":["zsa","senadmin",0],"/addadmin":["senadmin","admin",1],"/deladmin":["senadmin","admin",0],"/addsenmoder":["admin","senmoder",1],"/delsenmoder":["admin","senmoder",0],"/addmoder":["senmoder","moder",1],"/delmoder":["senmoder","moder",0]};const [need,rr,on]=map[c];if(!await can(peer,from,need))return send(peer,"Недостаточно прав.");const t=await target(a);if(!t)return send(peer,"Укажите @username.");if(RANK[await role(peer,t)]>=RANK[await role(peer,from)]&&t!==from)return send(peer,"Нельзя изменить роль пользователя равного или выше вас.");if(c.includes("sa")||c.includes("zsa")&&false)await S.setGlobalRole(t,on?rr:"none");else if(rr==="zsa"||rr==="sa")await S.setGlobalRole(t,on?rr:"none");else await S.setRole(peer,t,on?rr:"none");return send(peer,on?`${ROLE_RU[rr]} назначен.`:`${ROLE_RU[rr]} снят.`);
 }
...
async function handleEvent(body:any){
 if(body.type==="message_event"){
  const o=body.object,p=o.peer_id,u=o.user_id;let pl:any;try{pl=typeof o.payload==="string"?JSON.parse(o.payload):o.payload}catch{return};await vk("messages.sendMessageEventAnswer",{event_id:o.event_id,user_id:u,peer_id:p});if(pl?.a==="type"&&await can(p,u,"senadmin")){await S.setChatType(p,pl.v);return send(p,`Вы установили тип беседы "${pl.v==="admin"?"Административный чат":"Беседа игроков"}"`)}return;
 }
 if(body.type!=="message_new")return;const m=body.object.message;if(m.out)return;const p=m.peer_id,u=m.from_id,t=(m.text??"").trim();if(!isChat(p))return;await S.setChatInfo(p,`Беседа ${p-2000000000}`,await owner(p));
 ...
 // flood: five identical messages in ten seconds
...
Deno.serve(async req=>{if(req.method!=="POST")return new Response("Bot is running");let b;try{b=await req.json()}catch{return new Response("bad",{status:400})}const secret=Deno.env.get("VK_SECRET");if(b.type==="confirmation")return new Response(Deno.env.get("VK_CONFIRMATION")??"");if(secret&&b.secret!==secret)return new Response("invalid secret",{status:403});try{await handleEvent(b)}catch(e){console.error(e)}return new Response("ok")});
