...
async function cmd(p:number,u:number,text:string){
 ...
 if(c==="/addgroup"||c==="/delgroup"){if(!await can(p,u,"senadmin"))return deny(p);const q=uid(a[0]??"")??p;if(!isChat(q))return send(p,"Укажите id беседы или используйте команду внутри беседы.");if(c==="/addgroup"&&!await S.isSync(q))return send(p,"Сначала выполните /sync в этой беседе.");if(c==="/addgroup"){await S.addMyGroup(u,q);return send(p,"Данная беседа добавлена в список ваших чатов")}await S.delMyGroup(u,q);return send(p,"Данная беседа удалена из списков ваших чатов")}
 ...
 if(["/reward","/balance","/stats","/pay","/top","/gtop"].includes(c)){if(await S.getChatType(p)!=="players")return send(p,"Команда доступна только в беседе игроков.");if(c==="/reward"){const n=await S.reward(p,u);return send(p,n===null?"Вы уже получали награду. Повторно можно через 3 часа.":`🎁 Вы получили ${n} монет!`)}const t=await target(p,a)??u;if(c==="/balance")return send(p,`Баланс ${mention(t)}: ${await S.balance(p,t)} монет`);if(c==="/stats"){const s=await S.stats(p,t);return send(p,`Информация о пользователе ${profile(t)}\nРоль: ${ROLE_RU[await role(p,t)]}\nБаланс: ${s.coins} монет\nНик: ${s.nick||"Нет"}\nВсего сообщений: ${s.messages}\nПоследнее сообщение: ${fmtDate(s.last)}`)}if(c==="/pay"){const to=await target(p,a),n=Number(a[1]);if(!to||!Number.isInteger(n)||n<=0)return send(p,"Формат: /pay @username количество");return send(p,await S.pay(p,u,to,n)?`Вы передали ${n} монет пользователю ${mention(to)}.`:"Недостаточно монет.")}if(c==="/top"){const rows=await S.top(p);return send(p,"🏆 Топ по балансу в чате:\n\n"+(rows.length?rows.map((x,i)=>`${i+1}. ${mention(x.id)} — ${x.coins} монет`).join("\n"):"Пока пусто"))}const rows=await S.globalTop();return send(p,"🌎 Топ среди всех пользователей:\n\n"+(rows.length?rows.map((x,i)=>`${i+1}. ${mention(x.id)} — ${x.coins} монет`).join("\n"):"Пока пусто"))}

 if(c==="/ban"||c==="/unban"||c==="/sban"||c==="/sunban"||c==="/gban"||c==="/gunban"){const t=await target(p,a),reason=a.slice(1).join(" ")||"Не указана";if(!t)return send(p,`Формат: ${c} @username причина`);if(c==="/ban"){if(!await can(p,u,"senmoder"))return deny(p);await S.addBan(String(p),t,u,reason);await kick(p,t);return send(p,`Пользователь ${mention(t)} заблокирован в этой беседе.`)}if(c==="/unban"){if(!await can(p,u,"senadmin"))return deny(p);await S.delBan(String(p),t);return send(p,`Блокировка ${mention(t)} снята в этой беседе.`)}if(c==="/sban"){if(!await can(p,u,"admin"))return deny(p);for(const q of await groups(u)){await S.addBan(String(q),t,u,reason);await kick(q,t)}return send(p,`Пользователь ${mention(t)} заблокирован во всех ваших беседах.`)}if(c==="/sunban"){if(!await can(p,u,"senadmin"))return deny(p);for(const q of await groups(u))await S.delBan(String(q),t);return send(p,`Блокировка ${mention(t)} снята во всех ваших беседах.`)}if(c==="/gban"){if(!await can(p,u,"zsa"))return deny(p);await S.addBan("global",t,u,reason);for(const q of await S.syncChats()){await S.addBan(String(q),t,u,reason);await kick(q,t)}return send(p,`Пользователь ${mention(t)} глобально заблокирован.`)}if(!await can(p,u,"zsa"))return deny(p);await S.delBan("global",t);for(const q of await S.syncChats())await S.delBan(String(q),t);return send(p,`Глобальная блокировка ${mention(t)} снята.`)}
 if(c==="/getban"||c==="/banlist"){if(!await can(p,u,"moder"))return deny(p);const t=await target(p,a)??u,scopes=await S.banScopes(t);return send(p,`Информация о блокировках пользователя ${profile(t)}\n\nГлобальная блокировка — ${scopes.includes("global")?"активна":"отсутствует"} (/gban)\nБлокировка во всех беседах — ${scopes.some(x=>x!=="global")?"есть":"отсутствует"} (/sban)\nБлокировки в беседах — ${scopes.some(x=>x!=="global")?"есть":"отсутствуют"} (/ban)`)}
 if(c==="/kick"||c==="/skick"||c==="/gkick"){const t=await target(p,a);if(!t)return send(p,`Формат: ${c} @username причина`);if(c==="/kick"&&!await can(p,u,"moder"))return deny(p);if(c==="/skick"&&!await can(p,u,"admin"))return deny(p);if(c==="/gkick"&&!await can(p,u,"zsa"))return deny(p);const ps=c==="/kick"?[p]:c==="/skick"?await groups(u):await S.syncChats();for(const q of ps)await kick(q,t);return send(p,`Пользователь ${mention(t)} исключён.`)}
 if(c==="/clear"){if(!await can(p,u,"moder"))return deny(p);const t=await target(p,a);if(!t)return send(p,"Формат: /clear @username");if(RANK[await role(p,t)]>=RANK[await role(p,u)])return send(p,"Нельзя очистить сообщения пользователя с равным или более высоким рангом.");const h=await vk("messages.getHistory",{peer_id:p,count:200});const ids=(h?.response?.items??[]).filter((x:any)=>Number(x.from_id)===t&&x.conversation_message_id).map((x:any)=>x.conversation_message_id).slice(0,100);if(ids.length)await vk("messages.delete",{peer_id:p,cmids:ids.join(","),delete_for_all:1});return send(p,`Удалено сообщений: ${ids.length}.`)}
 if(c==="/mute"||c==="/unmute"){if(!await can(p,u,"moder"))return deny(p);const t=await target(p,a);if(!t)return send(p,`Формат: ${c} @username`);if(c==="/unmute"){await S.delMute(p,t);return send(p,`Мут с пользователя ${mention(t)} снят.`)}const n=Number(a[1]);if(!Number.isFinite(n)||n<=0)return send(p,"Укажите время в минутах.");await S.addMute(p,t,Date.now()+n*60000,a.slice(2).join(" ")||"Не указана",u);return send(p,`Пользователь ${mention(t)} получил мут на ${n} мин.`)}
 if(c==="/timeout"){if(!await can(p,u,"admin"))return deny(p);const on=!a[0]||["on","1","вкл","включить"].includes(a[0].toLowerCase());await S.timeout(p,on);return send(p,on?"🔇 Режим тишины включён.":"🔊 Режим тишины выключен.")}
 if(["/setnick","/getnick","/getacc","/removenick","/nlist"].includes(c)){if(!await can(p,u,"moder"))return deny(p);if(c==="/setnick"){const t=await target(p,a),n=a.slice(1).join(" ");if(!t||!n)return send(p,"Формат: /setnick @username Nick_Name");await S.setNick(p,t,n);return send(p,`Ник пользователя ${mention(t)} установлен: ${n}`)}if(c==="/removenick"){const t=await target(p,a);if(!t)return send(p,"Формат: /removenick @username");await S.removeNick(p,t);return send(p,`Ник пользователя ${mention(t)} удалён.`)}if(c==="/getnick"){const t=await target(p,a);if(!t)return send(p,"Формат: /getnick @username");return send(p,`Ник пользователя ${mention(t)}: ${await S.getNick(p,t)??"Нет"}`)}if(c==="/getacc"){const t=await S.getUserIdByNick(p,a.join(" "));return send(p,t?`Пользователь: ${profile(t)}`:"Пользователь с таким ником не найден.")}const ns=await S.allNicks(p);return send(p,"Список ников:\n"+(ns.length?ns.map(x=>`${x.nick} — ${mention(x.id)}`).join("\n"):"Отсутствуют"))}
...
async function handle(body:any){
 ...
 }catch(e){console.error(`[BOT ERROR] peer=${p} from=${u}`,e)}
}

Deno.serve(async req=>{
 if(req.method!=="POST")return new Response("Astana Manager OK",{status:200});
 ...
 if(b?.type==="message_new"||b?.type==="message_event")void handle(b).catch(e=>console.error("[WEBHOOK FATAL]",e));
