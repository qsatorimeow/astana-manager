...
export async function callVkApi(method:string,params:Record<string,string|number|undefined>={}):Promise<any>{
  ...
  p.set("access_token",TOKEN); p.set("v",VERSION);
  try{
    ...
    const d=await r.json(); if(d.error) console.error(`[VK API] ${method}`,JSON.stringify(d.error)); return d;
...
export const getConversationMembers=getMembers;

export async function getUsersInfo(ids:number[]){
  const out=new Map<number,any>();const x=[...new Set(ids.filter(Boolean))];if(!x.length)return out;
  const d=await callVkApi("users.get",{user_ids:x.join(",")});for(const u of d?.response??[])out.set(Number(u.id),u);return out;
}
...
export async function resolveTargetUserId(raw:string):Promise<number|null>{
  let s=raw.trim();if(!s)return null;let m=s.match(/\[id(\d+)\|/);if(m)return Number(m[1]);
  s=s.replace(/^https?:\/\/(www\.)?(vk\.com|vk\.ru)\//i,"").replace(/^@/,"");m=s.match(/^id(\d+)$/i);if(m)return Number(m[1]);if(/^\d+$/.test(s))return Number(s);
  const d=await callVkApi("utils.resolveScreenName",{screen_name:s});return d?.response?.type==="user"?Number(d.response.object_id):null;
}

async function getGlobalMessageId(peer:number,cmid:number):Promise<number|null>{
  if(!cmid)return null;
  const d=await callVkApi("messages.getByConversationMessageId",{peer_id:peer,conversation_message_ids:String(cmid)});
  const id=Number(d?.response?.items?.[0]?.id??0);return id>0?id:null;
}
...
export async function sendMessageAndGetIds(peer:number,text:string,opts:{replyTo?:number;keyboard?:any}={}):Promise<{messageId?:number;conversationMessageId?:number}>{
  ...
  if(opts.replyTo){
    const globalId=await getGlobalMessageId(peer,Number(opts.replyTo));
    if(globalId) p.reply_to=globalId;
    else p.forward=JSON.stringify({peer_id:peer,conversation_message_ids:[Number(opts.replyTo)],is_reply:1});
  }
  ...
  console.log(`[VK SEND] peer=${peer} reply_to_cmid=${opts.replyTo??"-"} global_reply=${p.reply_to??"-"} response=${JSON.stringify(d?.response??null)}`);
  const id=Number(d?.response??0);return id>0?{messageId:id}:{};
}
export const send=sendMessageAndGetIds;

export async function deleteMessages(peer:number,cmids:number[]){const xs=cmids.filter(x=>Number(x)>0);if(xs.length)await callVkApi("messages.delete",{peer_id:peer,cmids:xs.join(","),delete_for_all:1})}
