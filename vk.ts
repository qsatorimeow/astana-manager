...
/**
 * Отправляет сообщение как визуальный "ответ" (reply) на конкретное сообщение
 * в этой же беседе — используем forward с is_reply, т.к. conversation_message_id
 * приходит в вебхуке надёжно (в отличие от глобального message_id).
 */
...
/**
 * Отправляет сообщение и возвращает его message_id и conversation_message_id.
 * В этой конфигурации ответ messages.send не содержит рабочего id (response: 0),
 * поэтому id всегда достаём через messages.getHistory сразу после отправки.
 */
export async function sendMessageAndGetIds(
  ...
  if (lastMessage?.out === 1) {
    ...
    return {
      ...
      conversationMessageId: Number.isNaN(conversationMessageId) || conversationMessageId <= 0
        ? undefined
...
export async function getUsersInfo(userIds: number[]): Promise<Map<number, VkUserInfo>> {
  ...
  const data = await callVkApi("users.get", {
    user_ids: positiveIds.join(","),
  });
...
export async function getConversationMembers(peerId: number): Promise<ConversationMember[]> {
  const data = await callVkApi("messages.getConversationMembers", {
    peer_id: String(peerId),
  });
